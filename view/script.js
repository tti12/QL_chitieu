...
function doLogout() {
  ...
  // hide profile panel and actions
...
function showMainForUser(username) {
  ...
  // show profile action and populate simple profile name
  document.getElementById('profileBtn')?.classList.remove('hidden');
  const pn = document.getElementById('profileName');
  if (pn && (!pn.innerText || pn.innerText === '')) pn.innerText = username;
}

function updateProfileUI(user) {
  try {
    const name = (user && (user.name || user.username)) || getCurrentUser() || '';
    const email = (user && user.email) || '';
    const pn = document.getElementById('profileName');
    const pe = document.getElementById('profileEmail');
    if (pn) pn.innerText = name;
    if (pe) pe.innerText = email;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // legacy header logout removed — use profile panel logout

  document.getElementById("addExpenseBtn").addEventListener("click", addExpense);

  // profile button: toggle panel
  ...
  // profile logout
  document.getElementById('profileLogout')?.addEventListener('click', (ev) => {
    ev.preventDefault();
    profilePanel?.classList.add('hidden');
    profilePanel?.setAttribute('aria-hidden', 'true');
    doLogout();
  ...
  // Floating label support: toggle .filled when input has value
  const floatInputs = document.querySelectorAll('.input-group input');
  floatInputs.forEach(inp => {
    const toggle = () => {
      if (inp.value && inp.value.toString().trim() !== '') inp.classList.add('filled');
